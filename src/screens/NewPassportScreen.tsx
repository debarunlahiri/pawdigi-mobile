...
export function NewPassportScreen({
    ...
    breedsToFetch.forEach(async (item) => {
      try {
        const response = await fetch(
          `https://dog.ceo/api/breed/${dogCeoBreedPaths[item]}/images/random`,
        );
  ...
  const handleContinue = () => {
    ...
    const nextErrors = getPassportErrors({
      petName,
      breed: nextBreed,
      species,
      birthDate,
  ...
  return (
    <View style={styles.screen}>
      ...
      <View
        style={[
          styles.inputWrap,
          showErrors && errors.petName && styles.inputError,
        ]}
      >
        <TextInput
          style={styles.input}
      ...
      <View style={styles.breedField}>
        <Pressable
          style={[
            styles.inputWrap,
            styles.breedInputWrap,
            showErrors && errors.breed && styles.inputError,
          ]}
          onPress={() => {
            setBreedQuery("");
...
function resolveBreedSelection({
  ...
  const exactMatch = breedOptions[species].find(
