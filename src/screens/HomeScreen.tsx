...
import {
  GuardianAccessScreen,
  IdentificationScreen,
  VaccinationRecordsScreen,
} from "./PassportDetailScreens";
...
const samplePets: HomePet[] = [
  {
    ...
    gender: "Female",
    isSterilized: true,
    photoUri: "https://images.dog.ceo/breeds/husky/n02110185_1469.jpg",
    microchipNumber: "423424432",
    weight: "12",
    weightUnit: "kg",
  },
  {
    ...
    gender: "Female",
    isSterilized: true,
    photoUri: "https://images.dog.ceo/breeds/husky/n02110185_10116.jpg",
    microchipNumber: "",
    weight: "24",
    weightUnit: "kg",
  },
...
export function HomeScreen({
  ...
  const [passportDetail, setPassportDetail] = useState<
    "identification" | "vaccinations" | "guardian" | null
  >(null);
  const [showStory, setShowStory] = useState(false);
  const profileTransition = useRef(new Animated.Value(1)).current;
  const createdPets = (pets ?? []).filter((item) => item.name.trim());
  const hasCreatedPet = Boolean(pet?.name.trim());
  const dogs: HomePet[] =
    createdPets.length > 0
      ...
      : hasCreatedPet && pet
        ? [pet]
        : samplePets;
  const activePet = dogs[selectedDog] ?? dogs[0];
  ...
  useEffect(() => {
    if (selectedDog >= dogs.length) {
      setSelectedDog(0);
    }
  }, [dogs.length, selectedDog]);
  ...
  if (passportDetail === "identification" && activePet)
    return (
      <IdentificationScreen
        pet={activePet}
        onBack={() => setPassportDetail(null)}
      />
    );
  if (passportDetail === "vaccinations")
    return <VaccinationRecordsScreen onBack={() => setPassportDetail(null)} />;
  if (passportDetail === "guardian")
    return <GuardianAccessScreen onBack={() => setPassportDetail(null)} />;
  ...
  return (
    <View style={styles.screen}>
      <HomeHeader
        ...
        onAvatarPress={() => setShowStory(true)}
        onSwitchDog={() => setShowDogSwitcher(true)}
      ...
      <Animated.View
        style={[
          styles.profileContent,
          {
            ...
            transform: [
              {
                translateX: profileTransition.interpolate({
                  inputRange: [0, 1],
                  outputRange: [18, 0],
                }),
              },
              {
                scale: profileTransition.interpolate({
                  inputRange: [0, 1],
                  outputRange: [0.98, 1],
                }),
              },
      ...
      >
        {selectedTab === "passport" && activePet ? (
          <PassportFragment
            pet={activePet}
            onMedicalHistory={() => setShowMedicalRecords(true)}
            onIdentification={() => setPassportDetail("identification")}
            onVaccinations={() => setPassportDetail("vaccinations")}
            onGuardian={() => setPassportDetail("guardian")}
          />
        ) : selectedTab === "dogs" ? (
          <DogsFragment
            dogs={dogs}
            ...
            onAddDog={onAddFamilyMember}
          />
        ) : selectedTab === "alerts" ? (
          <AlertsFragment dogs={dogs} />
        ) : selectedTab === "profile" ? (
          <ProfileFragment pet={activePet} onSignOut={onSignOut} />
          ...
          <View style={styles.homeCard}>
            ...
            <Text style={styles.homeTitle}>
              {selectedTab === "home" ? "PawDigi Home" : tabLabel(selectedTab)}
            </Text>
            <Text style={styles.homeSubtitle}>
              {selectedTab === "home"
                ? "My profile setup is complete."
                : `${tabLabel(selectedTab)} section`}
            </Text>
          </View>
        )}
      </Animated.View>
      ...
      <DogSwitcher
        ...
        onSelect={(index) => {
          ...
          Animated.timing(profileTransition, {
            ...
            setSelectedDog(index);
            Animated.spring(profileTransition, {
              toValue: 1,
              friction: 8,
              tension: 90,
              useNativeDriver: true,
      ...
      <DogStoryViewer
        visible={showStory}
        dogName={activePet?.name || "My Story"}
        avatarUri={activePet?.photoUri}
          ...
          >
            <View
              style={[
                ...
                selected && styles.navigationIconSelected,
              ]}
            >
              <FontAwesome5
                name={item.icon}
                size={20}
                color={selected ? colors.card : colors.body}
              />
            </View>
          </Pressable>
        );
...
function HomeHeader({
  ...
  return (
    <View style={styles.header}>
      <Pressable style={styles.storyRing} onPress={onAvatarPress}>
        <Image
  ...
  return (
    ...
    >
      <Pressable style={styles.modalBackdrop} onPress={onClose}>
        <Pressable
          style={styles.switchSheet}
          onPress={(event) => event.stopPropagation()}
        >
          <View style={styles.sheetHandle} />
          <View style={styles.sheetHeader}>
            <View>
              <Text style={styles.sheetTitle}>My Family</Text>
              <Text style={styles.sheetSubtitle}>
                Choose whose profile is active
              </Text>
            </View>
            <Pressable style={styles.closeButton} onPress={onClose}>
              <FontAwesome5 name="times" size={16} color={colors.body} />
            </Pressable>
          </View>
          {dogs.map((dog, index) => {
            ...
            return (
              <Pressable
                key={`${dog.name}-${index}`}
                style={[styles.dogOption, selected && styles.dogOptionSelected]}
          ...
          <AddFamilyMemberButton onPress={onAdd} style={styles.addDogOption} />
        </Pressable>
      </Pressable>
    </Modal>
  );
    ...
    >
      <View style={styles.passportCard}>
        ...
        <View style={styles.qrBadge}>
          <FontAwesome5 name="qrcode" size={28} color={colors.primary} />
        </View>
        ...
        <View style={styles.microchipBox}>
          ...
          <Text style={styles.microchip}>
            {formatMicrochip(pet.microchipNumber)}
          </Text>
        </View>
      </View>
      ...
      <MenuCard
        ...
        subtitle="My breed, markings, and physical traits"
        onPress={onIdentification}
      />
      <View style={styles.sectionCard}>
        <View style={styles.sectionHeading}>
          <View style={styles.menuIcon}>
            <FontAwesome5 name="syringe" size={19} color={colors.primary} />
          </View>
          <View>
            <Text style={styles.menuTitle}>Vaccination Record</Text>
            <Text style={styles.menuSubtitle}>3 active, 1 upcoming</Text>
          </View>
          <Pressable style={styles.viewAllButton} onPress={onVaccinations}>
      ...
      <MenuCard
        ...
        subtitle="Surgery logs, clinical reports, and notes"
        onPress={onMedicalHistory}
      />
      <MenuCard
        icon="user-friends"
        title="Guardian Access"
        subtitle="Manage sharing with vets and family"
        onPress={onGuardian}
      />

      <Pressable style={styles.pdfButton}>
        <FontAwesome5 name="file-pdf" size={19} color={colors.card} />
        <Text style={styles.pdfText}>Generate PDF Passport</Text>
      </Pressable>
      <Pressable style={styles.shareButton}>
        <FontAwesome5 name="share-alt" size={18} color={colors.primary} />
        <Text style={styles.shareText}>Share Secure Link</Text>
      </Pressable>
    </ScrollView>
  );
}
...
function MenuCard({
  ...
  return (
    <Pressable style={styles.menuCard} onPress={onPress}>
      <View style={styles.menuIcon}>
        <FontAwesome5 name={icon} size={19} color={colors.primary} />
      </View>
      <View style={styles.menuCopy}>
        <Text style={styles.menuTitle}>{title}</Text>
        <Text style={styles.menuSubtitle}>{subtitle}</Text>
      </View>
      <FontAwesome5 name="chevron-right" size={16} color={colors.body} />
    </Pressable>
  );
}
  ...
  return (
    <View style={styles.vaccine}>
      ...
      <View style={styles.menuCopy}>
        ...
        <Text style={styles.menuSubtitle}>{date}</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  pdfText: { color: colors.card, fontFamily: fontFamily.bold, fontSize: 15 },
  shareButton: {
    marginTop: 8,
    height: 50,
    borderRadius: 11,
