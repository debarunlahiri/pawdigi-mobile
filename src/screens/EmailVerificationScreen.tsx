...
export function EmailVerificationScreen({ mode, onBackPress, onVerified }: EmailVerificationScreenProps) {
  ...
  const handleResend = () => {
    ...
    setMessage(mode === 'reset' ? 'Reset link resent.' : 'Verification email resent.');
  };
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.card}>
        <Animated.View style={[styles.iconHalo, { opacity: pulseOpacity, transform: [{ scale: pulseScale }] }]}>
        ...
        <Pressable onPress={handleOpenEmail} style={({ pressed }) => [styles.primaryButton, pressed && styles.primaryButtonPressed]}>
          <FontAwesome5 name="external-link-alt" size={18} color="#FFFFFF" />
          <Text style={styles.primaryButtonText}>Open Email App</Text>
        </Pressable>

        <View style={styles.waitingPill}>
          <Animated.View style={[styles.waitingDot, { opacity: pulseOpacity }]} />
      ...
      <Pressable style={styles.backRow} onPress={onBackPress}>
        ...
        <Text style={styles.backText}>Back to {mode === 'register' ? 'Register' : 'Reset Password'}</Text>
      </Pressable>
